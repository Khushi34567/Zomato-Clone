const restaurants = [
  {
    "image": "seven",
    "name": "Bamboo Alley",
    "rating": "3.9",
    "food_type": "Korean",
    "price_for_two": 830,
    "location": "Greater Kailash",
    "distance": "3.4",
    "offers": 16,
    "alcohol": false,
    "restaurant_open_time": 14,
    "restaurant_close_time": 2
  },
  {
    "image": "seven",
    "name": "Iron Skillet Social",
    "rating": "5.1",
    "food_type": "Thai",
    "price_for_two": 305,
    "location": "Lajpat Nagar",
    "distance": "9.4",
    "offers": 8,
    "alcohol": false,
    "restaurant_open_time": 8,
    "restaurant_close_time": 20
  },
  {
    "image": "fifth",
    "name": "Olive & Thyme",
    "rating": "1.9",
    "food_type": "Mexican",
    "price_for_two": 1713,
    "location": "Hauz Khas",
    "distance": "2.0",
    "offers": 21,
    "alcohol": true,
    "restaurant_open_time": 16,
    "restaurant_close_time": 4
  },
  {
    "image": "sixth",
    "name": "The Velvet Kebab",
    "rating": "4.6",
    "food_type": "Thai",
    "price_for_two": 2219,
    "location": "Defence Colony",
    "distance": "1.3",
    "offers": 15,
    "alcohol": true,
    "restaurant_open_time": 8,
    "restaurant_close_time": 20
  },
  {
    "image": "seven",
    "name": "Green Fig Bistro",
    "rating": "5.7",
    "food_type": "Italian",
    "price_for_two": 2170,
    "location": "Saket",
    "distance": "10.1",
    "offers": 17,
    "alcohol": false,
    "restaurant_open_time": 14,
    "restaurant_close_time": 2
  },
  {
    "image": "fourth",
    "name": "The Rolling Samosa",
    "rating": "2.9",
    "food_type": "Middle Eastern",
    "price_for_two": 1388,
    "location": "Chandni Chowk",
    "distance": "5.7",
    "offers": 7,
    "alcohol": true,
    "restaurant_open_time": 23,
    "restaurant_close_time": 11
  },
  {
    "image": "seven",
    "name": "The Brass Fork",
    "rating": "1.2",
    "food_type": "French",
    "price_for_two": 1463,
    "location": "Greater Kailash",
    "distance": "5.7",
    "offers": 15,
    "alcohol": false,
    "restaurant_open_time": 4,
    "restaurant_close_time": 16
  },
  {
    "image": "first",
    "name": "Starlight Diner",
    "rating": "5.2",
    "food_type": "Italian",
    "price_for_two": 875,
    "location": "Vasant Vihar",
    "distance": "4.5",
    "offers": 4,
    "alcohol": false,
    "restaurant_open_time": 7,
    "restaurant_close_time": 19
  },
  {
    "image": "seven",
    "name": "Sunset Pho",
    "rating": "2.2",
    "food_type": "Chinese",
    "price_for_two": 2027,
    "location": "Chandni Chowk",
    "distance": "2.5",
    "offers": 16,
    "alcohol": false,
    "restaurant_open_time": 3,
    "restaurant_close_time": 15
  },
  {
    "image": "fourth",
    "name": "Ember & Oak",
    "rating": "5.5",
    "food_type": "Mexican",
    "price_for_two": 463,
    "location": "Connaught Place",
    "distance": "7.9",
    "offers": 19,
    "alcohol": false,
    "restaurant_open_time": 14,
    "restaurant_close_time": 2
  },
  {
    "image": "first",
    "name": "Saffron & Stone",
    "rating": "3.8",
    "food_type": "Italian",
    "price_for_two": 2131,
    "location": "Greater Kailash",
    "distance": "6.3",
    "offers": 7,
    "alcohol": false,
    "restaurant_open_time": 3,
    "restaurant_close_time": 15
  },
  {
    "image": "sixth",
    "name": "Maple Grove Grill",
    "rating": "3.0",
    "food_type": "Greek",
    "price_for_two": 1779,
    "location": "Greater Kailash",
    "distance": "6.9",
    "offers": 3,
    "alcohol": false,
    "restaurant_open_time": 15,
    "restaurant_close_time": 3
  },
  {
    "image": "sixth",
    "name": "Blue Juniper",
    "rating": "1.6",
    "food_type": "French",
    "price_for_two": 799,
    "location": "Saket",
    "distance": "9.9",
    "offers": 3,
    "alcohol": false,
    "restaurant_open_time": 15,
    "restaurant_close_time": 3
  },
  {
    "image": "first",
    "name": "Starlight Diner",
    "rating": "1.1",
    "food_type": "Greek",
    "price_for_two": 1930,
    "location": "Chandni Chowk",
    "distance": "4.8",
    "offers": 15,
    "alcohol": false,
    "restaurant_open_time": 13,
    "restaurant_close_time": 1
  },
  {
    "image": "sixth",
    "name": "Cielo Taquería",
    "rating": "3.0",
    "food_type": "Thai",
    "price_for_two": 360,
    "location": "Jor Bagh",
    "distance": "6.8",
    "offers": 20,
    "alcohol": false,
    "restaurant_open_time": 15,
    "restaurant_close_time": 3
  },
  {
    "image": "eigth",
    "name": "Iron Skillet Social",
    "rating": "2.2",
    "food_type": "Indian",
    "price_for_two": 690,
    "location": "Jor Bagh",
    "distance": "6.6",
    "offers": 17,
    "alcohol": true,
    "restaurant_open_time": 1,
    "restaurant_close_time": 13
  },
  {
    "image": "eigth",
    "name": "Sunset Pho",
    "rating": "5.3",
    "food_type": "Thai",
    "price_for_two": 359,
    "location": "Kashmiri Gate",
    "distance": "3.4",
    "offers": 0,
    "alcohol": false,
    "restaurant_open_time": 12,
    "restaurant_close_time": 0
  },
  {
    "image": "eigth",
    "name": "Iron Skillet Social",
    "rating": "5.3",
    "food_type": "Thai",
    "price_for_two": 1987,
    "location": "Vasant Vihar",
    "distance": "6.7",
    "offers": 12,
    "alcohol": false,
    "restaurant_open_time": 0,
    "restaurant_close_time": 12
  },
  {
    "image": "eigth",
    "name": "Green Fig Bistro",
    "rating": "4.0",
    "food_type": "Middle Eastern",
    "price_for_two": 1356,
    "location": "Saket",
    "distance": "1.9",
    "offers": 8,
    "alcohol": false,
    "restaurant_open_time": 17,
    "restaurant_close_time": 5
  },
  {
    "image": "eigth",
    "name": "Olive & Thyme",
    "rating": "6.0",
    "food_type": "Japanese",
    "price_for_two": 1903,
    "location": "Vasant Vihar",
    "distance": "2.2",
    "offers": 3,
    "alcohol": false,
    "restaurant_open_time": 13,
    "restaurant_close_time": 1
  },
  {
    "image": "third",
    "name": "Harbor Lantern",
    "rating": "2.0",
    "food_type": "Chinese",
    "price_for_two": 676,
    "location": "Defence Colony",
    "distance": "8.1",
    "offers": 9,
    "alcohol": true,
    "restaurant_open_time": 16,
    "restaurant_close_time": 4
  },
  {
    "image": "seven",
    "name": "Masa y Mar",
    "rating": "4.2",
    "food_type": "French",
    "price_for_two": 976,
    "location": "Jor Bagh",
    "distance": "5.4",
    "offers": 13,
    "alcohol": false,
    "restaurant_open_time": 12,
    "restaurant_close_time": 0
  },
  {
    "image": "sixth",
    "name": "Sunset Pho",
    "rating": "4.8",
    "food_type": "Middle Eastern",
    "price_for_two": 2460,
    "location": "Vasant Vihar",
    "distance": "10.0",
    "offers": 21,
    "alcohol": false,
    "restaurant_open_time": 4,
    "restaurant_close_time": 16
  },
  {
    "image": "ninth",
    "name": "The Brass Fork",
    "rating": "3.7",
    "food_type": "Italian",
    "price_for_two": 360,
    "location": "Kashmiri Gate",
    "distance": "5.6",
    "offers": 7,
    "alcohol": false,
    "restaurant_open_time": 15,
    "restaurant_close_time": 3
  },
  {
    "image": "sixth",
    "name": "Maple Grove Grill",
    "rating": "2.5",
    "food_type": "Thai",
    "price_for_two": 2497,
    "location": "Chandni Chowk",
    "distance": "7.0",
    "offers": 17,
    "alcohol": false,
    "restaurant_open_time": 23,
    "restaurant_close_time": 11
  },
  {
    "image": "ninth",
    "name": "Ember & Oak",
    "rating": "1.0",
    "food_type": "Japanese",
    "price_for_two": 1804,
    "location": "Vasant Vihar",
    "distance": "1.3",
    "offers": 13,
    "alcohol": false,
    "restaurant_open_time": 4,
    "restaurant_close_time": 16
  },
  {
    "image": "third",
    "name": "Masa y Mar",
    "rating": "5.8",
    "food_type": "Greek",
    "price_for_two": 211,
    "location": "Lajpat Nagar",
    "distance": "4.7",
    "offers": 19,
    "alcohol": false,
    "restaurant_open_time": 0,
    "restaurant_close_time": 12
  },
  {
    "image": "third",
    "name": "La Piraña",
    "rating": "2.6",
    "food_type": "Indian",
    "price_for_two": 1200,
    "location": "Jor Bagh",
    "distance": "7.0",
    "offers": 15,
    "alcohol": false,
    "restaurant_open_time": 13,
    "restaurant_close_time": 1
  },
  {
    "image": "fifth",
    "name": "Harbor Lantern",
    "rating": "5.5",
    "food_type": "Japanese",
    "price_for_two": 1881,
    "location": "Defence Colony",
    "distance": "1.1",
    "offers": 16,
    "alcohol": false,
    "restaurant_open_time": 19,
    "restaurant_close_time": 7
  },
  {
    "image": "tenth",
    "name": "Bamboo Alley",
    "rating": "4.7",
    "food_type": "Thai",
    "price_for_two": 667,
    "location": "Vasant Vihar",
    "distance": "5.1",
    "offers": 9,
    "alcohol": false,
    "restaurant_open_time": 17,
    "restaurant_close_time": 5
  },
  {
    "image": "ninth",
    "name": "Nori & Naan",
    "rating": "3.1",
    "food_type": "Indian",
    "price_for_two": 2282,
    "location": "Lajpat Nagar",
    "distance": "8.9",
    "offers": 2,
    "alcohol": false,
    "restaurant_open_time": 20,
    "restaurant_close_time": 8
  },
  {
    "image": "fourth",
    "name": "Cielo Taquería",
    "rating": "2.3",
    "food_type": "Indian",
    "price_for_two": 1191,
    "location": "Jor Bagh",
    "distance": "5.8",
    "offers": 11,
    "alcohol": false,
    "restaurant_open_time": 0,
    "restaurant_close_time": 12
  },
  {
    "image": "fourth",
    "name": "Sunset Pho",
    "rating": "4.8",
    "food_type": "Thai",
    "price_for_two": 813,
    "location": "Hauz Khas",
    "distance": "6.1",
    "offers": 19,
    "alcohol": false,
    "restaurant_open_time": 21,
    "restaurant_close_time": 9
  },
  {
    "image": "second",
    "name": "Cielo Taquería",
    "rating": "2.2",
    "food_type": "Middle Eastern",
    "price_for_two": 123,
    "location": "Kashmiri Gate",
    "distance": "4.8",
    "offers": 10,
    "alcohol": false,
    "restaurant_open_time": 18,
    "restaurant_close_time": 6
  },
  {
    "image": "fourth",
    "name": "La Piraña",
    "rating": "4.4",
    "food_type": "Indian",
    "price_for_two": 460,
    "location": "Vasant Vihar",
    "distance": "5.1",
    "offers": 26,
    "alcohol": false,
    "restaurant_open_time": 2,
    "restaurant_close_time": 14
  },
  {
    "image": "third",
    "name": "La Piraña",
    "rating": "3.4",
    "food_type": "Italian",
    "price_for_two": 2276,
    "location": "Defence Colony",
    "distance": "1.4",
    "offers": 5,
    "alcohol": false,
    "restaurant_open_time": 6,
    "restaurant_close_time": 18
  },
  {
    "image": "first",
    "name": "Lotus Pearl",
    "rating": "4.0",
    "food_type": "Japanese",
    "price_for_two": 2279,
    "location": "Defence Colony",
    "distance": "1.6",
    "offers": 20,
    "alcohol": false,
    "restaurant_open_time": 2,
    "restaurant_close_time": 14
  },
  {
    "image": "sixth",
    "name": "Nori & Naan",
    "rating": "1.2",
    "food_type": "Greek",
    "price_for_two": 2458,
    "location": "Hauz Khas",
    "distance": "8.5",
    "offers": 4,
    "alcohol": false,
    "restaurant_open_time": 19,
    "restaurant_close_time": 7
  },
  {
    "image": "ninth",
    "name": "Cielo Taquería",
    "rating": "1.3",
    "food_type": "Middle Eastern",
    "price_for_two": 336,
    "location": "Jor Bagh",
    "distance": "9.1",
    "offers": 1,
    "alcohol": false,
    "restaurant_open_time": 0,
    "restaurant_close_time": 12
  },
  {
    "image": "fifth",
    "name": "Bamboo Alley",
    "rating": "3.2",
    "food_type": "Italian",
    "price_for_two": 1132,
    "location": "Jor Bagh",
    "distance": "1.0",
    "offers": 22,
    "alcohol": false,
    "restaurant_open_time": 8,
    "restaurant_close_time": 20
  },
  {
    "image": "fourth",
    "name": "Green Fig Bistro",
    "rating": "2.2",
    "food_type": "Thai",
    "price_for_two": 1394,
    "location": "Chandni Chowk",
    "distance": "1.2",
    "offers": 23,
    "alcohol": false,
    "restaurant_open_time": 9,
    "restaurant_close_time": 21
  },
  {
    "image": "tenth",
    "name": "Masa y Mar",
    "rating": "4.7",
    "food_type": "Korean",
    "price_for_two": 461,
    "location": "Jor Bagh",
    "distance": "4.0",
    "offers": 28,
    "alcohol": false,
    "restaurant_open_time": 5,
    "restaurant_close_time": 17
  },
  {
    "image": "sixth",
    "name": "Sunset Pho",
    "rating": "3.0",
    "food_type": "Mexican",
    "price_for_two": 788,
    "location": "Saket",
    "distance": "6.0",
    "offers": 0,
    "alcohol": false,
    "restaurant_open_time": 9,
    "restaurant_close_time": 21
  },
  {
    "image": "eigth",
    "name": "Iron Skillet Social",
    "rating": "3.1",
    "food_type": "Greek",
    "price_for_two": 414,
    "location": "Greater Kailash",
    "distance": "2.8",
    "offers": 8,
    "alcohol": false,
    "restaurant_open_time": 14,
    "restaurant_close_time": 2
  },
  {
    "image": "fourth",
    "name": "Starlight Diner",
    "rating": "4.0",
    "food_type": "Japanese",
    "price_for_two": 271,
    "location": "Kashmiri Gate",
    "distance": "1.9",
    "offers": 11,
    "alcohol": false,
    "restaurant_open_time": 0,
    "restaurant_close_time": 12
  },
  {
    "image": "second",
    "name": "Ember & Oak",
    "rating": "1.6",
    "food_type": "Korean",
    "price_for_two": 2295,
    "location": "Jor Bagh",
    "distance": "9.1",
    "offers": 29,
    "alcohol": true,
    "restaurant_open_time": 11,
    "restaurant_close_time": 23
  },
  {
    "image": "seven",
    "name": "Blue Juniper",
    "rating": "4.8",
    "food_type": "Chinese",
    "price_for_two": 106,
    "location": "Vasant Vihar",
    "distance": "2.3",
    "offers": 1,
    "alcohol": false,
    "restaurant_open_time": 11,
    "restaurant_close_time": 23
  },
  {
    "image": "third",
    "name": "Lotus Pearl",
    "rating": "5.9",
    "food_type": "Thai",
    "price_for_two": 1396,
    "location": "Defence Colony",
    "distance": "10.3",
    "offers": 26,
    "alcohol": false,
    "restaurant_open_time": 6,
    "restaurant_close_time": 18
  },
  {
    "image": "ninth",
    "name": "Maple Grove Grill",
    "rating": "1.2",
    "food_type": "Greek",
    "price_for_two": 2127,
    "location": "Chandni Chowk",
    "distance": "2.0",
    "offers": 28,
    "alcohol": false,
    "restaurant_open_time": 17,
    "restaurant_close_time": 5
  },
  {
    "image": "tenth",
    "name": "Blue Juniper",
    "rating": "4.4",
    "food_type": "Greek",
    "price_for_two": 522,
    "location": "Kashmiri Gate",
    "distance": "6.1",
    "offers": 6,
    "alcohol": false,
    "restaurant_open_time": 17,
    "restaurant_close_time": 5
  },
  {
    "image": "sixth",
    "name": "Cielo Taquería",
    "rating": "4.1",
    "food_type": "Mexican",
    "price_for_two": 1799,
    "location": "Greater Kailash",
    "distance": "10.3",
    "offers": 10,
    "alcohol": false,
    "restaurant_open_time": 21,
    "restaurant_close_time": 9
  },
  {
    "image": "second",
    "name": "The Rolling Samosa",
    "rating": "2.3",
    "food_type": "Italian",
    "price_for_two": 1276,
    "location": "Vasant Vihar",
    "distance": "6.9",
    "offers": 8,
    "alcohol": false,
    "restaurant_open_time": 3,
    "restaurant_close_time": 15
  },
  {
    "image": "eigth",
    "name": "The Velvet Kebab",
    "rating": "4.5",
    "food_type": "Thai",
    "price_for_two": 1770,
    "location": "Lajpat Nagar",
    "distance": "2.8",
    "offers": 21,
    "alcohol": true,
    "restaurant_open_time": 11,
    "restaurant_close_time": 23
  },
  {
    "image": "sixth",
    "name": "Cielo Taquería",
    "rating": "1.2",
    "food_type": "Middle Eastern",
    "price_for_two": 952,
    "location": "Jor Bagh",
    "distance": "6.6",
    "offers": 25,
    "alcohol": false,
    "restaurant_open_time": 8,
    "restaurant_close_time": 20
  },
  {
    "image": "second",
    "name": "Blue Juniper",
    "rating": "4.1",
    "food_type": "Middle Eastern",
    "price_for_two": 1412,
    "location": "Lajpat Nagar",
    "distance": "8.5",
    "offers": 4,
    "alcohol": false,
    "restaurant_open_time": 4,
    "restaurant_close_time": 16
  },
  {
    "image": "second",
    "name": "Mama Rosa Trattoria",
    "rating": "4.3",
    "food_type": "Middle Eastern",
    "price_for_two": 972,
    "location": "Chandni Chowk",
    "distance": "1.4",
    "offers": 6,
    "alcohol": false,
    "restaurant_open_time": 15,
    "restaurant_close_time": 3
  },
  {
    "image": "eigth",
    "name": "Mama Rosa Trattoria",
    "rating": "5.0",
    "food_type": "Italian",
    "price_for_two": 1782,
    "location": "Connaught Place",
    "distance": "5.7",
    "offers": 6,
    "alcohol": false,
    "restaurant_open_time": 20,
    "restaurant_close_time": 8
  },
  {
    "image": "third",
    "name": "Bamboo Alley",
    "rating": "4.7",
    "food_type": "Korean",
    "price_for_two": 772,
    "location": "Jor Bagh",
    "distance": "6.4",
    "offers": 26,
    "alcohol": false,
    "restaurant_open_time": 6,
    "restaurant_close_time": 18
  },
  {
    "image": "eigth",
    "name": "Maple Grove Grill",
    "rating": "2.0",
    "food_type": "Italian",
    "price_for_two": 2106,
    "location": "Hauz Khas",
    "distance": "7.4",
    "offers": 1,
    "alcohol": false,
    "restaurant_open_time": 11,
    "restaurant_close_time": 23
  },
  {
    "image": "fourth",
    "name": "Mama Rosa Trattoria",
    "rating": "5.5",
    "food_type": "Korean",
    "price_for_two": 2255,
    "location": "Jor Bagh",
    "distance": "8.1",
    "offers": 14,
    "alcohol": false,
    "restaurant_open_time": 16,
    "restaurant_close_time": 4
  },
  {
    "image": "third",
    "name": "Lotus Pearl",
    "rating": "5.3",
    "food_type": "French",
    "price_for_two": 2441,
    "location": "Saket",
    "distance": "10.6",
    "offers": 29,
    "alcohol": false,
    "restaurant_open_time": 13,
    "restaurant_close_time": 1
  },
  {
    "image": "fifth",
    "name": "Starlight Diner",
    "rating": "5.2",
    "food_type": "Chinese",
    "price_for_two": 1217,
    "location": "Vasant Vihar",
    "distance": "1.3",
    "offers": 8,
    "alcohol": false,
    "restaurant_open_time": 4,
    "restaurant_close_time": 16
  },
  {
    "image": "ninth",
    "name": "Lotus Pearl",
    "rating": "4.2",
    "food_type": "Chinese",
    "price_for_two": 1993,
    "location": "Hauz Khas",
    "distance": "2.3",
    "offers": 12,
    "alcohol": false,
    "restaurant_open_time": 19,
    "restaurant_close_time": 7
  },
  {
    "image": "tenth",
    "name": "Sunset Pho",
    "rating": "4.9",
    "food_type": "Thai",
    "price_for_two": 1614,
    "location": "Chandni Chowk",
    "distance": "2.1",
    "offers": 13,
    "alcohol": true,
    "restaurant_open_time": 5,
    "restaurant_close_time": 17
  },
  {
    "image": "third",
    "name": "Sunset Pho",
    "rating": "5.7",
    "food_type": "Middle Eastern",
    "price_for_two": 2121,
    "location": "Connaught Place",
    "distance": "9.1",
    "offers": 23,
    "alcohol": false,
    "restaurant_open_time": 9,
    "restaurant_close_time": 21
  },
  {
    "image": "first",
    "name": "Cielo Taquería",
    "rating": "1.1",
    "food_type": "Greek",
    "price_for_two": 771,
    "location": "Defence Colony",
    "distance": "1.1",
    "offers": 8,
    "alcohol": false,
    "restaurant_open_time": 21,
    "restaurant_close_time": 9
  },
  {
    "image": "seven",
    "name": "Masa y Mar",
    "rating": "3.0",
    "food_type": "French",
    "price_for_two": 687,
    "location": "Greater Kailash",
    "distance": "4.8",
    "offers": 22,
    "alcohol": false,
    "restaurant_open_time": 11,
    "restaurant_close_time": 23
  },
  {
    "image": "seven",
    "name": "Green Fig Bistro",
    "rating": "2.5",
    "food_type": "Chinese",
    "price_for_two": 1412,
    "location": "Hauz Khas",
    "distance": "4.7",
    "offers": 1,
    "alcohol": false,
    "restaurant_open_time": 0,
    "restaurant_close_time": 12
  },
  {
    "image": "first",
    "name": "Saffron & Stone",
    "rating": "1.8",
    "food_type": "Middle Eastern",
    "price_for_two": 694,
    "location": "Lajpat Nagar",
    "distance": "9.2",
    "offers": 15,
    "alcohol": false,
    "restaurant_open_time": 13,
    "restaurant_close_time": 1
  },
  {
    "image": "ninth",
    "name": "Blue Juniper",
    "rating": "5.1",
    "food_type": "Korean",
    "price_for_two": 698,
    "location": "Greater Kailash",
    "distance": "1.6",
    "offers": 10,
    "alcohol": true,
    "restaurant_open_time": 14,
    "restaurant_close_time": 2
  },
  {
    "image": "first",
    "name": "Green Fig Bistro",
    "rating": "4.4",
    "food_type": "Chinese",
    "price_for_two": 535,
    "location": "Connaught Place",
    "distance": "4.2",
    "offers": 9,
    "alcohol": false,
    "restaurant_open_time": 3,
    "restaurant_close_time": 15
  },
  {
    "image": "third",
    "name": "Bamboo Alley",
    "rating": "4.7",
    "food_type": "Greek",
    "price_for_two": 1734,
    "location": "Lajpat Nagar",
    "distance": "2.8",
    "offers": 1,
    "alcohol": false,
    "restaurant_open_time": 14,
    "restaurant_close_time": 2
  },
  {
    "image": "second",
    "name": "The Rolling Samosa",
    "rating": "2.1",
    "food_type": "Indian",
    "price_for_two": 1537,
    "location": "Chandni Chowk",
    "distance": "4.8",
    "offers": 13,
    "alcohol": false,
    "restaurant_open_time": 16,
    "restaurant_close_time": 4
  },
  {
    "image": "eigth",
    "name": "Ember & Oak",
    "rating": "3.5",
    "food_type": "Indian",
    "price_for_two": 1325,
    "location": "Hauz Khas",
    "distance": "4.9",
    "offers": 29,
    "alcohol": false,
    "restaurant_open_time": 8,
    "restaurant_close_time": 20
  },
  {
    "image": "fourth",
    "name": "Masa y Mar",
    "rating": "1.8",
    "food_type": "French",
    "price_for_two": 1320,
    "location": "Jor Bagh",
    "distance": "5.0",
    "offers": 6,
    "alcohol": true,
    "restaurant_open_time": 15,
    "restaurant_close_time": 3
  },
  {
    "image": "eigth",
    "name": "La Piraña",
    "rating": "4.6",
    "food_type": "Korean",
    "price_for_two": 1448,
    "location": "Saket",
    "distance": "6.4",
    "offers": 20,
    "alcohol": false,
    "restaurant_open_time": 11,
    "restaurant_close_time": 23
  },
  {
    "image": "second",
    "name": "Maple Grove Grill",
    "rating": "2.8",
    "food_type": "Japanese",
    "price_for_two": 1103,
    "location": "Chandni Chowk",
    "distance": "3.7",
    "offers": 9,
    "alcohol": false,
    "restaurant_open_time": 22,
    "restaurant_close_time": 10
  },
  {
    "image": "fifth",
    "name": "Sunset Pho",
    "rating": "3.7",
    "food_type": "Indian",
    "price_for_two": 2252,
    "location": "Hauz Khas",
    "distance": "9.0",
    "offers": 8,
    "alcohol": false,
    "restaurant_open_time": 3,
    "restaurant_close_time": 15
  },
  {
    "image": "tenth",
    "name": "The Brass Fork",
    "rating": "1.4",
    "food_type": "Korean",
    "price_for_two": 1746,
    "location": "Hauz Khas",
    "distance": "10.6",
    "offers": 20,
    "alcohol": false,
    "restaurant_open_time": 0,
    "restaurant_close_time": 12
  },
  {
    "image": "fourth",
    "name": "Olive & Thyme",
    "rating": "5.3",
    "food_type": "Chinese",
    "price_for_two": 1063,
    "location": "Greater Kailash",
    "distance": "9.2",
    "offers": 20,
    "alcohol": true,
    "restaurant_open_time": 3,
    "restaurant_close_time": 15
  },
  {
    "image": "ninth",
    "name": "Bamboo Alley",
    "rating": "3.2",
    "food_type": "Mexican",
    "price_for_two": 2177,
    "location": "Jor Bagh",
    "distance": "9.0",
    "offers": 13,
    "alcohol": false,
    "restaurant_open_time": 20,
    "restaurant_close_time": 8
  },
  {
    "image": "ninth",
    "name": "Ember & Oak",
    "rating": "4.4",
    "food_type": "French",
    "price_for_two": 1849,
    "location": "Jor Bagh",
    "distance": "4.3",
    "offers": 10,
    "alcohol": true,
    "restaurant_open_time": 12,
    "restaurant_close_time": 0
  },
  {
    "image": "first",
    "name": "Saffron & Stone",
    "rating": "5.8",
    "food_type": "Middle Eastern",
    "price_for_two": 845,
    "location": "Hauz Khas",
    "distance": "8.6",
    "offers": 28,
    "alcohol": true,
    "restaurant_open_time": 17,
    "restaurant_close_time": 5
  },
  {
    "image": "eigth",
    "name": "La Piraña",
    "rating": "2.1",
    "food_type": "Mexican",
    "price_for_two": 1797,
    "location": "Kashmiri Gate",
    "distance": "1.9",
    "offers": 17,
    "alcohol": false,
    "restaurant_open_time": 1,
    "restaurant_close_time": 13
  },
  {
    "image": "sixth",
    "name": "The Brass Fork",
    "rating": "3.6",
    "food_type": "Thai",
    "price_for_two": 2376,
    "location": "Chandni Chowk",
    "distance": "10.7",
    "offers": 22,
    "alcohol": true,
    "restaurant_open_time": 23,
    "restaurant_close_time": 11
  },
  {
    "image": "fourth",
    "name": "Mama Rosa Trattoria",
    "rating": "4.8",
    "food_type": "Thai",
    "price_for_two": 2062,
    "location": "Greater Kailash",
    "distance": "8.0",
    "offers": 14,
    "alcohol": false,
    "restaurant_open_time": 9,
    "restaurant_close_time": 21
  },
  {
    "image": "fifth",
    "name": "The Brass Fork",
    "rating": "5.3",
    "food_type": "Japanese",
    "price_for_two": 1745,
    "location": "Chandni Chowk",
    "distance": "4.3",
    "offers": 27,
    "alcohol": false,
    "restaurant_open_time": 8,
    "restaurant_close_time": 20
  },
  {
    "image": "ninth",
    "name": "Maple Grove Grill",
    "rating": "1.8",
    "food_type": "Greek",
    "price_for_two": 281,
    "location": "Hauz Khas",
    "distance": "2.4",
    "offers": 10,
    "alcohol": true,
    "restaurant_open_time": 4,
    "restaurant_close_time": 16
  },
  {
    "image": "ninth",
    "name": "Saffron & Stone",
    "rating": "4.2",
    "food_type": "Middle Eastern",
    "price_for_two": 106,
    "location": "Jor Bagh",
    "distance": "8.3",
    "offers": 17,
    "alcohol": false,
    "restaurant_open_time": 14,
    "restaurant_close_time": 2
  },
  {
    "image": "eigth",
    "name": "Masa y Mar",
    "rating": "3.3",
    "food_type": "French",
    "price_for_two": 2416,
    "location": "Hauz Khas",
    "distance": "3.3",
    "offers": 25,
    "alcohol": false,
    "restaurant_open_time": 13,
    "restaurant_close_time": 1
  },
  {
    "image": "first",
    "name": "The Rolling Samosa",
    "rating": "5.6",
    "food_type": "French",
    "price_for_two": 437,
    "location": "Lajpat Nagar",
    "distance": "7.9",
    "offers": 22,
    "alcohol": false,
    "restaurant_open_time": 22,
    "restaurant_close_time": 10
  },
  {
    "image": "third",
    "name": "Cielo Taquería",
    "rating": "2.0",
    "food_type": "Chinese",
    "price_for_two": 904,
    "location": "Jor Bagh",
    "distance": "7.2",
    "offers": 13,
    "alcohol": false,
    "restaurant_open_time": 9,
    "restaurant_close_time": 21
  },
  {
    "image": "fourth",
    "name": "The Rolling Samosa",
    "rating": "2.5",
    "food_type": "Middle Eastern",
    "price_for_two": 1645,
    "location": "Defence Colony",
    "distance": "5.6",
    "offers": 8,
    "alcohol": false,
    "restaurant_open_time": 8,
    "restaurant_close_time": 20
  },
  {
    "image": "third",
    "name": "Blue Juniper",
    "rating": "4.6",
    "food_type": "French",
    "price_for_two": 2339,
    "location": "Lajpat Nagar",
    "distance": "8.8",
    "offers": 21,
    "alcohol": true,
    "restaurant_open_time": 0,
    "restaurant_close_time": 12
  },
  {
    "image": "seven",
    "name": "Cielo Taquería",
    "rating": "4.5",
    "food_type": "Japanese",
    "price_for_two": 560,
    "location": "Kashmiri Gate",
    "distance": "10.3",
    "offers": 22,
    "alcohol": false,
    "restaurant_open_time": 10,
    "restaurant_close_time": 22
  },
  {
    "image": "third",
    "name": "The Rolling Samosa",
    "rating": "5.1",
    "food_type": "Japanese",
    "price_for_two": 1992,
    "location": "Greater Kailash",
    "distance": "7.7",
    "offers": 3,
    "alcohol": true,
    "restaurant_open_time": 2,
    "restaurant_close_time": 14
  },
  {
    "image": "fourth",
    "name": "Blue Juniper",
    "rating": "3.8",
    "food_type": "Thai",
    "price_for_two": 562,
    "location": "Jor Bagh",
    "distance": "2.0",
    "offers": 22,
    "alcohol": false,
    "restaurant_open_time": 9,
    "restaurant_close_time": 21
  },
  {
    "image": "fourth",
    "name": "Masa y Mar",
    "rating": "4.2",
    "food_type": "Italian",
    "price_for_two": 1743,
    "location": "Vasant Vihar",
    "distance": "4.7",
    "offers": 5,
    "alcohol": true,
    "restaurant_open_time": 8,
    "restaurant_close_time": 20
  },
  {
    "image": "tenth",
    "name": "Ember & Oak",
    "rating": "3.1",
    "food_type": "Greek",
    "price_for_two": 1585,
    "location": "Connaught Place",
    "distance": "2.2",
    "offers": 25,
    "alcohol": false,
    "restaurant_open_time": 9,
    "restaurant_close_time": 21
  },
  {
    "image": "fourth",
    "name": "Saffron & Stone",
    "rating": "2.8",
    "food_type": "Indian",
    "price_for_two": 672,
    "location": "Connaught Place",
    "distance": "4.4",
    "offers": 27,
    "alcohol": false,
    "restaurant_open_time": 20,
    "restaurant_close_time": 8
  },
  {
    "image": "tenth",
    "name": "Starlight Diner",
    "rating": "2.3",
    "food_type": "Italian",
    "price_for_two": 2336,
    "location": "Defence Colony",
    "distance": "10.0",
    "offers": 14,
    "alcohol": false,
    "restaurant_open_time": 7,
    "restaurant_close_time": 19
  }
]
// how to make a card

function getrestaurant(restaurants){
  // const root = document.getElementById('root');
  const root = document.getElementById('root');
   restaurants.forEach(restaurant =>{
    //create a cart
    // image
    //cart content 
    //  1.card header name and rating
    //    2. card footer  food type or price
    //    3.card location  restaurant location, distamne



    //create a card div
    const card = document.createElement('div');
    card.classList.add('card');

    //create image 
    const image = document.createElement('img');
    image.src = `Images/${restaurant.image}.jpg`;
    // console.log(`Images/${restaurant.image}.jpg`);

    //card content
    const Card_content = document.createElement('div');
    Card_content.classList.add('card-content');

    //card header
    const Card_header = document.createElement('div');
    Card_header.classList.add('card-header');
    // name and rating
    const h3 = document.createElement('h3');
    h3.textContent = restaurant.name;

    const rate = document.createElement('span');
    rate.textContent = "Rating: " +restaurant.rating;
    rate.classList.add('rating');

    // card header ka ander dalne ka liye name or rating ko
    Card_header.appendChild(h3);
    Card_header.appendChild(rate);




    //card footer
    const Card_footer = document.createElement('div');
    Card_footer.classList.add('card-footer');

    // food types and price
    const food = document.createElement('span');
    food.textContent = restaurant.food_type;

    const price  = document.createElement('span');
    price.textContent = "₹" + restaurant.price_for_two;
    // const food = document.createElement('span');
    // food.classList.add('card-food-type');
    // food.textContent = restaurant.food_type;

    // const price = document.createElement('span');
    // price.classList.add('card-price');
    // price.textContent = restaurant.price_for_two;


    Card_footer.appendChild(food);
    Card_footer.appendChild(price);


    //card location
    const Card_location = document.createElement('div');
    Card_location.classList.add('card-location');

    //// distance and time 
    const locspan = document.createElement('span');
    locspan.textContent = restaurant.location;

    const distspan  = document.createElement('span');
    distspan.textContent = restaurant.distance+ "km";
 
    Card_location.appendChild(locspan);
    Card_location.appendChild(distspan);

    // yeh child ha card content ko to hme inhe bhi append krenge

    Card_content.appendChild(Card_header);
    Card_content.appendChild(Card_footer);
    Card_content.appendChild(Card_location);


    card.appendChild(image)
    card.appendChild(Card_content);

    root.appendChild(card);

    
   })
}




getrestaurant(restaurants)


document.getElementById("Alcohol Serves").addEventListener('click',()=>{
    const result =  restaurants.filter((obj)=>obj.alcohol);
    document.getElementById('root').innerHTML = ""; // replaceChildren();
    getrestaurant(result);

})
document.getElementById("Rating").addEventListener('click',()=>{
    const result =  restaurants.filter((obj)=>obj.rating > 4.5);
    document.getElementById('root').innerHTML = ""; // replaceChildren();
    getrestaurant(result);

})
document.getElementById("Offers").addEventListener('click',()=>{
    const result =  restaurants.filter((obj)=>obj.offers > 10);
    document.getElementById('root').innerHTML = ""; // replaceChildren();
    getrestaurant(result);

})
document.getElementById("Open").addEventListener('click',()=>{
    const result =  restaurants.filter((obj)=>obj.restaurant_open_time > 12);
    document.getElementById('root').innerHTML = ""; // replaceChildren();
    getrestaurant(result);

})

document.getElementById("Filters").addEventListener('click',()=>{
   
  document.getElementById("filterPopup").classList.remove("hidden");
})
document.getElementById("applyFilter").addEventListener('click',()=>{
   
   const element = document.querySelector('input[name="filterOption"]:checked');
   const answer = element.value;
   if(answer==="rating"){
    restaurants.sort((a,b)=>b.rating-a.rating);
   }
   else if(answer==="hightolow"){
    restaurants.sort((a,b)=>b.price_for_two-a.price_for_two);
   }
   else if(answer==="costLowtoHigh"){
    restaurants.sort((a,b)=>a.price_for_two-b.price_for_two);
   }
   else if(answer==="distance"){
    restaurants.sort((a,b)=>a.distance-b.distance);
   }

   document.getElementById('root').innerHTML = ""; // replaceChildren();
   document.getElementById("filterPopup").classList.add("hidden");
   getrestaurant(restaurants);

   
   

})




